...
while True:
    ...
    if option == "1": # Begining the play by taking the option "1"
        ...
        for player_name, player_profile in player_data.items():
            total_wins = player_profile.get("Total Win", 0)
        ...
        if most_wins == 0:
            print("\nNo player has won any games yet...")
        else:
            if len(top_players) > 1:
                print(f"\nWe have multiple winners/ties with {most_wins} wins:")
                for player in top_players:
                    print('\033[92m' + player + '\033[0m')
            else:
                print(f"\033[92m{top_players[0]}\033[0m showed some incredible skill with amazing {most_wins} wins today!")
