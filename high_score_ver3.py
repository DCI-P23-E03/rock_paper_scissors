        ...
        if (len(top_players)) > 1:  # Checking if top_players list is longer than one player
                print(f"There is tie at the moment with {most_wins} wins. Tied players are:")
                for player in top_players:  # For loop for displaying all tied players
                    print(player)
                    
        else:   # If top_players list length is 1, message for one sole winner is displayed
                for player in top_players:  
                    print(f"{player} showed some incredible skill with amazing {most_wins} wins today!") #For looping top_players list and displaying highest scoring player(s)
